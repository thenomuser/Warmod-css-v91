...
//	http://www.gametech.com.au/warmod/ << Offline
//
// Ported By @TheNomUser  Disscord > NomUser:9116
//
//
 ...
 *	>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
 *
 *	i edit this sh*t in one day whith 2 players
 *	i cant replace the deprecated vars beacuse -_-
 *	report bugs on github or discord > NomUser:9116
 *		
 *********************************************************/
...
/* cvars */
//new Handle:g_h_lw_enabled = INVALID_HANDLE;			//Livewire shit
//new Handle:g_h_lw_address = INVALID_HANDLE;			//Livewire shit
//new Handle:g_h_lw_port = INVALID_HANDLE;				//Livewire shit
//new Handle:g_h_lw_group_name = INVALID_HANDLE;		//Livewire shit
//new Handle:g_h_lw_group_password = INVALID_HANDLE;	//Livewire shit
...
/* livewire */
//new Handle:g_h_lw_socket = INVALID_HANDLE;	//Livewire
//new bool:g_lw_connecting = false;				//Livewire
//new bool:g_lw_connected = false;				//Livewire
...
CheckScores()
{
	...
	CS_SetTeamScore( TERRORIST_TEAM, GetTScore());
	CS_SetTeamScore( COUNTER_TERRORIST_TEAM, GetCTScore());
	SetTeamScore( TERRORIST_TEAM, GetTScore());
	SetTeamScore( COUNTER_TERRORIST_TEAM, GetCTScore());
}
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "");
		PrintToConsole(client, "Ported By @TheNomUser");
		PrintToConsole(client, "");
